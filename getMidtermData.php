...
while(	$rows	=	mysql_fetch_assoc($result) )
{
	...
	if(!$result1)
	{
		$reply = array(
				'success' 	=>		'0',
				'message' 	=>   	"<div id='error'>Midterm Error Occurred..</div>"
			);
	...
	if(mysql_num_rows($result1) > 0 )
	{
		...
		{
			$message	.=	"<div class='divExamList'><a class='examList' rel='$rows1[ExamDate]"."*"."$rows[SectionId]' id='MidtermsExist'>Midterm $MNo</a></div>";
			$MNo++;
		}
	...
	$message	.=	"<div class='divExamList'><a class='examList' rel='$rows[SectionId]' id='addNewMidterm'>Add Midterm</a></div>";
