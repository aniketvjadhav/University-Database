...
while(	$rows	=	mysql_fetch_assoc($result) )
{
	...
	if(!$result1)
	{
		$reply = array(
				'success' 	=>		'0',
				'message' 	=>   	"<div id='error'>Quiz Error Occurred..</div>"
			);
	...
	if(mysql_num_rows($result1) > 0 )
	{
		...
		{
			$message	.=	"<div class='divExamList'><a class='examList' rel='$rows1[ExamDate]"."*"."$rows[SectionId]' id='QuizExist'>Quiz $QNo</a></div>";
			$QNo++;
		}
	...
	$message	.=	"<div class='divExamList'><a class='examList' rel='$rows[SectionId]' id='addNewQuiz'>Add Quiz</a></div>";
